...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5522997060905@s.whatsapp.net"] // replace this with your number
			const isGroup = from.endsWith('@g.br')
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falhou, no momento da conversa ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'clearall':
					...
					reply('botroj deletou todo o chat :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é você?')
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('usa o ddd animal')
					try {
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marca o otário que você quer banir!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `lista dos adm's do grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ repete❌')
					reply(mess.wait)
					...
					break
				case 'simi':
					if (args.length < 1) return reply('sei lakkkk?')
					teks = body.slice(5)
